...
# Aplikasi Streamlit
st.title('VISUALISASI BENCANA LONGSOR JAWA BARAT')
...
with col[0]:
    ...
    total_DESA_1 = df['KABUPATEN'].nunique()
...
with col[2]:
    ...
    df_population_sorted_no_geom = df_population_sorted.drop(columns=['LATITUDE', 'LONGITUDE'])
    ...
    st.dataframe(df_population_sorted_no_geom,
                 ...
                 column_config={
                     ...
                     "JUMLAH_LONGSOR": st.column_config.ProgressColumn(
                         ...
                         format="%f",
                         min_value=0,
                         max_value=max(df_population_sorted.JUMLAH_LONGSOR),
                     )}
                 )
